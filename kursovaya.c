...
int less(const record *a, const record *b)
{
    for (int i = 0; i < 31; i++)
    {
        unsigned char char_a = (unsigned char)a->FIO[i];
        unsigned char char_b = (unsigned char)b->FIO[i];
        if (char_a != char_b)
        {
            if (char_a < char_b)
            {
    ...
    for (int i = 0; i < 17; i++)
    {
        unsigned char char_a = (unsigned char)a->Street[i];
        unsigned char char_b = (unsigned char)b->Street[i];
...
void BinarySearch(int n, record **records, const char *searchKey, tData **head, tData **tail)
{
    ...
    while (index < n && strncasecmp(records[index]->FIO, searchKey, 3) == 0)
    {
        ...
        if (recordCopy != NULL) 
        {
            memcpy(recordCopy, records[index], sizeof(record));
            AddToQueue(head, tail, recordCopy);
        }
